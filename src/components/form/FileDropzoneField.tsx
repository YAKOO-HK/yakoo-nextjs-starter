...
export const FileDropzoneField = React.forwardRef<React.ElementRef<'input'>, FileDropzoneFieldProps>(
  function FileDropzoneField(
    { id, name, value = [], onChange, dropzoneProps = {}, isError, className, ...props },
    ref
  ) {
    ...
    return (
      ...
      >
        <input ref={ref} {...getInputProps()} name={name} id={id || genId} />
        {value?.length > 0 ? (
